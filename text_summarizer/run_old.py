...
def test(config):
    ...
    for i in range(data.shape[0]):
        ...
        with open('summarized_sample_articles', 'a', newline='') as csvfile:
            fieldnames = ['Publisher','articleBody']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)        
            writer.writerow({'Publisher':pub, 'articleBody':s})
    ...
    print("Number of Ids ", body_id)
    new_df = pd.DataFrame()
    new_df['Publisher'] = body_id
    new_df['articleBody'] = summarized_articles
    new_df.to_csv('summarized_sample_articles_2.csv')
