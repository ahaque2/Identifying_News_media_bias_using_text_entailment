...
def test(config):
    Best_Model = torch.load(config.test_model, map_location=torch.device('cpu'))
    Tokenizer = BertTokenizer.from_pretrained(config.model)

    #f_in = open(config.inputFile, 'r')
    data = pd.read_csv(config.inputFile)
    print(data.shape)
    ...
    for line in sent:
        ...
        summarized_article.append(texts[0])
        
        print("Original Snetence ", line)
        print("summarized sentence ", texts[0])

        '''

        print("text this" , texts[0], end=ed)
        print("Text again ", texts[0], file=f_top1)
        print(len(texts), file=f_topK)
        for i in range(len(texts)):
            print("Answer ", Answers[i][0], file=f_topK)
            print("Answer text ", texts[i], file=f_topK)
        '''
    #print("summarized_article ", summarized_article) 
    s = " ".join(summarized_article)
    print("Summarized Article ", s)
    summarized_articles.append(s)
    '''
    with open('data/summarized_articles.csv', 'a', newline='') as csvfile:
        fieldnames = ['BodyID','articleBody']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)        
        writer.writerow({'BodyID':idx, 'articleBody':s})
    '''
    
    print("Done for i = ", i, end = ed)
        
    #f_top1.close()
    #f_topK.close()
    print("Num of Summarized articles : ", len(summarized_articles))
    #print("Numbe rof Ids ", body_id)
    #new_df = pd.DataFrame()
    #new_df['Body ID'] = body_id
    #new_df['articleBody'] = summarized_articles
    #new_df.to_csv('data/summarized_train_body.csv')
